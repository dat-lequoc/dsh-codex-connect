...
describe('Codex Connect update metadata', () => {
  it('accepts the image-model-hint highlight kind', () => {
  ...
  it('accepts only the fixed release URL and safe public response fields in the browser parser', () => {
    ...
    expect(parseOpenAICodexUpdateResult({ ...valid, credential: 'must not pass through' })).toEqual(valid)
  })

  it('accepts update results without host compatibility metadata and ignores legacy fields', () => {
    expect(parseOpenAICodexUpdateResult({
      status: 'up-to-date',
      currentVersion: '0.1.0-alpha.4.14',
      latestVersion: '0.1.0-alpha.4.14',
      currentDshVersion: 'legacy-host-field',
